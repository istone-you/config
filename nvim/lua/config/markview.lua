...
markview.render = function (buffer, state, config)
	require("config.markview.actions").render(buffer, state, config);
end
...
```lua
local renderer = require("config.markview").strict_render;
...
markview.strict_render = {
	...
	render = function (self, buffer, max_lines, config)
		...
		buffer = buffer or vim.api.nvim_get_current_buf();
		max_lines = max_lines or require("config.markview.spec").get({ "preview", "max_buf_lines" }, { ignore_enable = true, fallback = 1000 }) --[[@as integer]];
...
markview.actions = {
	__index = function (_, key)
		return require("config.markview.actions")[key];
	end
};
...
markview.commands = {
	__index = function (_, key)
		...
		return require("config.markview.commands")[key];
	end
};
...
markview.setup = function (config)
	require("config.markview.spec").setup(config);
end
