...
md_str.update_cache = function ()
	...
	local spec = require("config.markview.spec");
...
md_str.autolink = function (match)
	...
	if md_str.cached_config and md_str.cached_config.uri_autolinks then
		...
		local config = require("config.markview.utils").match(md_str.cached_config.uri_autolinks, removed, {
			eval_args = {
				md_str.buffer,
				{
...
md_str.block_ref = function (match)
	...
	if md_str.cached_config and md_str.cached_config.block_references then
		---@type markview.config.markdown_inline.emails.opts
		local config = require("config.markview.utils").match(md_str.cached_config.block_references, removed, {
			eval_args = {
				md_str.buffer,
				{
...
md_str.embed = function (match)
	...
	if md_str.cached_config and md_str.cached_config.embed_files then
		---@type markview.config.markdown_inline.emails.opts
		local config = require("config.markview.utils").match(md_str.cached_config.embed_files, removed, {
			eval_args = {
				md_str.buffer,
				{
...
md_str.emoji = function (match)
	...
	--- length (e.g. "rocket" = 6).
	local symbols = require("config.markview.symbols");
...
md_str.entity = function (match)
	...
	local removed = string.gsub(match, "^&#?", ""):gsub(";$", "");
	return require("config.markview.entities").get(removed) or match;
...
md_str.internal = function (match)
	...
	if md_str.cached_config and md_str.cached_config.internal_links then
		---@type markview.config.markdown_inline.emails.opts
		local config = require("config.markview.utils").match(md_str.cached_config.internal_links, removed, {
			eval_args = {
				md_str.buffer,
				{
...
md_str.footnote = function (match)
	...
	if md_str.cached_config and md_str.cached_config.footnotes then
		...
		local config = require("config.markview.utils").match(md_str.cached_config.footnotes, label, {
			eval_args = {
				md_str.buffer,
				{
...
md_str.tag = function (match)
	...
	if md_str.cached_config and md_str.cached_config.tags then
		---@type markview.config.__inline?
		local config = require("config.markview.utils").match(md_str.cached_config.tags, label, {
			eval_args = {
				md_str.buffer,
				{
					class = "inline_tag",
...
md_str.email = function (match)
	...
	if md_str.cached_config and md_str.cached_config.emails then
		---@type markview.config.markdown_inline.emails.opts
		local config = require("config.markview.utils").match(md_str.cached_config.emails, removed, {
			eval_args = {
				md_str.buffer,
				{
...
md_str.highlight = function (match)
	...
	if md_str.cached_config and md_str.cached_config.highlights then
		---@type markview.config.markdown_inline.emails.opts
		local config = require("config.markview.utils").match(md_str.cached_config.highlights, removed, {
			eval_args = {
				md_str.buffer,
				{
...
md_str.hyperlink_no_src = function (match)
	...
	if md_str.cached_config and md_str.cached_config.hyperlinks then
		...
		local config = require("config.markview.utils").match(md_str.cached_config.hyperlinks, removed, {
			eval_args = {
				md_str.buffer,
				{
...
md_str.hyperlink_src = function (match)
	...
	if md_str.cached_config and md_str.cached_config.hyperlinks then
		---@type markview.config.markdown_inline.hyperlinks.opts
		local config = require("config.markview.utils").match(md_str.cached_config.hyperlinks, address, {
			eval_args = {
				md_str.buffer,
				{
					class = "inline_link_hyperlink",
...
md_str.img_no_src = function (match)
	...
	if md_str.cached_config and md_str.cached_config.images then
		...
		local config = require("config.markview.utils").match(md_str.cached_config.images, removed, {
			eval_args = {
				md_str.buffer,
				{
...
md_str.img_src = function (match)
	...
	if md_str.cached_config and md_str.cached_config.images then
		---@type markview.config.markdown_inline.images.opts
		local config = require("config.markview.utils").match(md_str.cached_config.images, address, {
			eval_args = {
				md_str.buffer,
				{
					class = "inline_link_image",
