local markdown = {};
local inline = require("config.markview.renderers.markdown_inline");

local spec = require("config.markview.spec");
local utils = require("config.markview.utils");

local filetypes = require("config.markview.filetypes");
...
markdown.atx_heading = function (buffer, item)
	...
	elseif config.style == "label" then
		...
		if win and config.align then
			local res = require("config.markview.renderers.markdown.tostring").tostring(buffer, item.text[1]):gsub("^#+%s", "");
...
markdown.code_block = function (buffer, item)
	...
	local function render_block ()
		...
		for l, line in ipairs(item.text) do
			local final = require("config.markview.renderers.common.visual_text").get_visual_text(item.language, line);
	...
	-- NOTE: Allow options to be more *flexible*.
	if item.uses_tab or not win or config.style == "simple" then
		render_simple();
...
markdown.table = function (buffer, item)
	...
	for _, col in ipairs(item.header) do
		if col.class == "column" then
			local _o = require("config.markview.renderers.markdown.tostring").tostring(buffer, col.text);
...
markdown.__block_quote = function (buffer, item)
	...
	for l = range.row_start, range.row_end - 1, 1  do
		...
		require("config.markview.wrap").wrap_indent(buffer, { row = l }, {
...
markdown.__list_item = function (buffer, item)
	...
	for _, l in ipairs(item.candidates) do
		require("config.markview.wrap").wrap_indent(buffer, {
...
markdown.render = function (buffer, content)
	...
	for _, item in ipairs(content or {}) do
		...
		if success == false then
			require("config.markview.health").print({
				kind = "ERR",
...
markdown.post_render = function (buffer, content)
	local custom = spec.get({ "renderers" }, { fallback = {} });
	vim.g.markview_sign_width = require("config.markview.wrap").sign_width(buffer, markdown.ns);
	...
	require("config.markview.wrap").render(buffer, markdown.ns);
end
