...
local SECTIONS = {
  ...
  { header = '🔧 カスタムキーマップ', color = 'ShortcutsBuffer', rows = {
    ...
    { 'Space q',         '現在のバッファを閉じる（未保存は残す）' },
    { 'Space Q',         'タブラインに出ているバッファをすべて閉じる（未保存は残す）' },
    ...
    { 'Space d',         '問題パネルを開閉（全ファイルの診断一覧、詳細は下のセクション）' },
    ...
    { '補完 Ctrl-Space', '補完メニューを手動で出す（インサートモード）' },
  ...
  { header = '🩺 問題パネル（Space d で開閉）', color = 'ShortcutsBuffer', rows = {
