...
T.describe('git_panel Files panel: commit/amend/misc keys', function()
  ...
  T.it('y copies the path of the entry under the cursor to the unnamed register', function()
    local dir = T.tmp_git_repo()
  ...
  T.it('S opens stash options; u stashes including untracked files', function()
    local dir = T.tmp_git_repo(function(d)
      T.write_file(d .. '/tracked.txt', { 't' })
      GP.git(d, { 'add', '.' })
      GP.git(d, { '-c', 'user.email=t@t', '-c', 'user.name=t', 'commit', '-qm', 'seed' })
    end)
    T.write_file(dir .. '/tracked.txt', { 't', 'dirty' })
    T.write_file(dir .. '/fresh.txt', { 'new' })
    GP.open(dir, false)
    GP.press('S')
    vim.wait(80)
    GP.press_modal('u') -- 未追跡も含める
    vim.wait(50)
    -- startinsertはheadlessではvim.wait後に持続しないので、iで明示的に挿入へ入る
    GP.press_modal('iwith untracked')
    GP.press_modal('<CR>')
    T.wait_until(function()
      return GP.git(dir, { 'stash', 'list' }).stdout:find('with untracked', 1, true) ~= nil
    end)
    T.contains(GP.git(dir, { 'stash', 'list' }).stdout, 'with untracked')
    T.eq(vim.trim(GP.git(dir, { 'status', '--porcelain' }).stdout), '')
    T.eq(vim.fn.filereadable(dir .. '/fresh.txt'), 0, 'untracked file should be stashed away')

    GP.close()
    T.rmrf(dir)
  end)

  T.it('S t stashes only staged changes, leaving unstaged edits', function()
    local dir = T.tmp_git_repo(function(d)
      T.write_file(d .. '/a.txt', { 'a' })
    ...
    GP.git(dir, { 'add', 'a.txt' })
    T.write_file(dir .. '/b.txt', { 'b', 'unstaged' })
    GP.open(dir, false)
    GP.press('S')
    vim.wait(80)
    GP.press_modal('t')
    vim.wait(50)
    GP.press_modal('istaged only')
    GP.press_modal('<CR>')
    T.wait_until(function()
      return GP.git(dir, { 'stash', 'list' }).stdout:find('staged only', 1, true) ~= nil
    end)
    T.contains(GP.git(dir, { 'stash', 'list' }).stdout, 'staged only')
    local status = GP.git(dir, { 'status', '--porcelain' }).stdout
    T.ok(status:find('b.txt', 1, true) ~= nil, 'unstaged b.txt should remain dirty')
    T.ok(not status:find('a.txt', 1, true), 'staged a.txt should be gone from status')
