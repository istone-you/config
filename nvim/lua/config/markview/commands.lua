...
commands.Start = function ()
	require("config.markview.state").enable(true);
end
commands.Stop = function ()
	require("config.markview.state").enable(false);
end


commands.disable = function (buffer)
	require("config.markview.actions").disable(buffer);
end
commands.enable = function (buffer)
	require("config.markview.actions").enable(buffer);
end
commands.toggle = function (buffer)
	require("config.markview.actions").toggle(buffer);
end


commands.Disable = function ()
	local actions = require("config.markview.actions");
	actions.forEach(actions.disable, require("config.markview.state").get_attached_buffers());
end
commands.Enable = function ()
	local actions = require("config.markview.actions");
	actions.forEach(actions.enable, require("config.markview.state").get_attached_buffers());
end
commands.Toggle = function ()
	local actions = require("config.markview.actions");
	actions.forEach(actions.toggle, require("config.markview.state").get_attached_buffers());
end


commands.attach = function (buffer)
	require("config.markview.actions").attach(buffer);
end
commands.detach = function (buffer)
	require("config.markview.actions").detach(buffer);
end


commands.clear = function (buffer)
	buffer = buffer or vim.api.nvim_get_current_buf();
	require("config.markview.actions").clear(buffer);
end
commands.render = function (buffer)
	buffer = buffer or vim.api.nvim_get_current_buf();
	require("config.markview.actions").render(buffer);
end
...
commands.Clear = function ()
	...
	actions.forEach(function (b)
		actions.clear(b);
	end, require("config.markview.state").get_attached_buffers());
end
commands.Render = function ()
	local actions = require("config.markview.actions");

	actions.forEach(function (b)
		local buf_state = require("config.markview.state").get_buffer_state(b, false);
...
commands.hybridDisable = function (buffer)
	require("config.markview.actions").hybridDisable(buffer);
end
commands.hybridEnable = function (buffer)
	require("config.markview.actions").hybridEnable(buffer);
end
commands.hybridToggle = function (buffer)
	---|fS

	buffer = buffer or vim.api.nvim_get_current_buf();
	local state = require("config.markview.state").get_buffer_state(buffer, false);
...
commands.linewiseDisable = function ()
	require("config.markview.spec").config.preview.linewise_hybrid_mode = false;
	local actions = require("config.markview.actions");

	actions.forEach(function (b)
		require("config.markview.actions").render(b);
	end, require("config.markview.state").get_enabled_buffers());
end
commands.linewiseEnable = function ()
	require("config.markview.spec").config.preview.linewise_hybrid_mode = true;
	local actions = require("config.markview.actions");
...
commands.linewiseToggle = function ()
	...
	local linewise_hybrid_mode = require("config.markview.spec").get({ "preview", "linewise_hybrid_mode" }, { fallback = false, ignore_enable = true });
...
commands.splitToggle = function ()
	require("config.markview.actions").splitToggle();
end

commands.splitRedraw = function ()
	require("config.markview.actions").splitview_render();
end

commands.splitOpen = function (buffer)
	require("config.markview.actions").splitOpen(buffer)
end

commands.splitClose = function ()
	require("config.markview.actions").splitClose()
end

--[[ Open `link` under cursor. ]]
commands.open = function ()
	require("config.markview.links").open();
end


commands.traceExport = function ()
	require("config.markview.health").export();
end
commands.traceShow = function ()
	require("config.markview.health").view();
end
...
commands.complete = function (_, cmd, cursorpos)
	...
	elseif is_subcommand(parts[2]) == true then
		local state = require("config.markview.state");
