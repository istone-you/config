...
asciidoc.block_quote = function (buffer, item)
	...
	if item.from then
		...
		if _context then
			context = require("config.markview.renderers.asciidoc.tostring").tostring(
				buffer,
				_context,
...
asciidoc.code_block = function (buffer, item)
	...
	local decorations = require("config.markview.filetypes").get(item.language);
...
asciidoc.toc = function (buffer, item)
	...
	for _, entry in ipairs(item.entries or {}) do
		...
		if config then
			local text = require("config.markview.renderers.asciidoc.tostring").tostring(buffer, entry.text, utils.set_hl(config.hl) --[[@as string]]);
