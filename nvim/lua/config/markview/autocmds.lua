...
autocmds.use_delay = function (buffer, event_name, args)
	...
	elseif event_name == "CursorMoved" or event_name == "CursorMovedI" then
		...
		---@type integer
		local max_l = require("config.markview.spec").get({ "preview", "max_buf_lines" }, { fallback = 1000, ignore_enable = true });
...
autocmds.modeChanged = function (args)
	...
	local function action ()
		...
		require("config.markview.health").print({ kind = "skip", back = true });
	end
...
autocmds.bufHandle = function (args)
	...
	elseif state.buf_attached(buf) then
		...
		if buf_state and buf_state.enable then
			...
			require("config.markview.actions").set_query(buf);
		end
	...
	require("config.markview.actions").attach(buf);
...
autocmds.optionSet = function (args)
	...
	if vim.list_contains({ "filetype", "buftype" }, args.match) then
		if autocmds.should_detach(args) then
			require("config.markview.actions").detach(args.buf);
		else
...
autocmds.cursor = function (args)
	...
	local delay = require("config.markview.spec").get({ "preview", "debounce" }, { fallback = 25, ignore_enable = true });
...
autocmds.lazy_loaded = function ()
	...
	require("config.markview.highlights").setup();
	require("config.markview.integrations").setup();
...
autocmds.setup = function ()
	...
	vim.api.nvim_create_autocmd({
		...
		callback = function(args)
			require("config.markview.state").detach(args.buf, true);
		end
	});
