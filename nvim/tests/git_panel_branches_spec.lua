...
T.describe('git_panel Branches panel', function()
  ...
  T.it('y copies the selected branch name to the clipboard register', function()
    local dir = T.tmp_git_repo()
    GP.git(dir, { 'branch', 'copy-me' })
    GP.open(dir, false)
    GP.press('3')
    vim.wait(200)
    local left = GP.left_win()
    local row = GP.find_row(left, 'copy-me')
    T.ok(row ~= nil)
    GP.goto_row(left, row)
    GP.press('y')
    vim.wait(50)
    T.eq(vim.fn.getreg('"'), 'copy-me')
