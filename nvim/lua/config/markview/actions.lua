...
actions.handle_hybrid_mode = function (linewise, buffer, line_count, wins, content)
	...
	local spec = require("config.markview.spec");
	local renderer = require("config.markview.renderer");
...
actions.clear = function (buffer)
	...
	require("config.markview.renderer").clear(buffer, 0, -1);
end
...
actions.render = function (_buffer, _state, _config)
	...
	local parser = require("config.markview.parser");
	local renderer = require("config.markview.renderer");
	local spec = require("config.markview.spec");
...
actions.clean = function ()
	...
	require("config.markview.state").clean(nil, function ()
...
actions.detach = function (buffer)
	...
	require("config.markview.state").detach(buffer, false);
	actions.clear(buffer);
	require("config.markview.health").print({ kind = "skip", back = true });
...
actions.traceExport = function ()
	...
	for _, entry in ipairs(require("config.markview.health").log) do
